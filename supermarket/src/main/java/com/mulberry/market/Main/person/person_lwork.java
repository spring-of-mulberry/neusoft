...
public class person_lwork {
    ...
    public void lwork(){
        ...
        try {
            ...
            attenceinfo1.setOffdutydate(sqldate);
        } catch (ParseException e) {
            e.printStackTrace();
        }
        attenceinfo1.setAttenceno(userno1);
        pwork2.lworkdaka(attenceinfo1);
        System.out.println("下班打卡已完成！！！自动返回到收银员系统");
        person.person_UI();
    }
...
//            可能错误：语法错误，编号存在
            ...
            if (!flag) {
                System.out.println("员工序号输入有误！请输入纯数字!");
                now_userno();
                break;
            }else {
                ...
                //判断表中是否有该用户
                boolean f =false;
                list2 = pwork3.worktimeByAttenceno1();
                ...
                if (f){
                    System.out.println("没有该用户编号的打卡记录！");
                    now_userno();
                    break;
                }else {
                    int offduty = 0;
                    list1 = pwork1.useroffduty(i, offduty);
                    if (list1.size() > 0) {
